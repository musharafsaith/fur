...
export default function Navbar({ onLangToggle, currentLang }: { onLangToggle: () => void, currentLang: string }) {
    ...
    <nav className="fixed top-0 left-0 w-full z-50 bg-brand-paper/80 backdrop-blur-md border-b border-brand-ink/10">
      <div className="max-w-7xl mx-auto px-6 h-20 flex items-center justify-between">
        ...
        <div className="flex items-center gap-6">
          ...
          <button className="hover:text-brand-accent transition-colors" id="search-btn">
            <Search size={20} strokeWidth={1.5} />
          </button>
          <button className="hover:text-brand-accent transition-colors relative" id="cart-btn">
      ...
      {isOpen && (
        ...
        >
          <a href="#collection" className="text-lg font-serif" onClick={() => setIsOpen(false)}>Collection</a>
          <a href="#about" className="text-lg font-serif" onClick={() => setIsOpen(false)}>Our Story</a>
          <a href="#contact" className="text-lg font-serif" onClick={() => setIsOpen(false)}>Contact</a>
        </motion.div>
      )}
    </nav>
  );
